...
class FindingsGetArguments(TaskArguments):
...
class FindingsGet(CommandBase):
    cmd = "findings_get"
    needs_admin = False
    help_cmd = "findings_get -title Penetration"
    description = "Get all the findings associated with a specific report"
    version = 2
    author = "@its_a_feature_"
    argument_class = FindingsGetArguments
    supported_ui_features = ["ghostwriter:findings_get"]
    browser_script = BrowserScript(script_name="findings_get", author="@its_a_feature_")
    attackmapping = []
    ...
    async def create_go_tasking(self,
        ...
        findings_get_query = gql(
            """
            query findingsGetForReport($title: String!, $projectId: bigint!) {
              reportedFinding(where: {report: {title: {_ilike: $title}, projectId: {_eq: $projectId}}}, order_by: {id: desc}) {
                complete
                cvssScore
                findingType {
                  findingType
        ...
        try:
            ...
            if title == "%%":
                title = "%_%"
            response_code, response_data = await GhostwriterAPI.query_graphql(taskData, query=findings_get_query,
